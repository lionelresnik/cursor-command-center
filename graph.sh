...
generate_mermaid() {
    ...
    # Generate Mermaid
    cat << EOF
%%{init: {'look': 'handDrawn', 'flowchart': {'defaultRenderer': 'elk'}}}%%
graph TB
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .zoom-btn:hover { background: rgba(0,217,255,0.3); }
        /* Panning viewport */
        .graph-viewport {
            overflow: hidden;
            cursor: grab;
        ...
        .beta-text { font-size: 0.85rem; opacity: 0.9; }
        .beta-text code {
        ...
        .node-dimmed > rect, .node-dimmed > polygon, .node-dimmed > circle,
        .node-dimmed > ellipse, .node-dimmed > path {
            opacity: 0.15 !important;
        }
        .node-dimmed .label, .node-dimmed text { opacity: 0.15 !important; }
        .edge-dimmed { opacity: 0.08 !important; }
        .node-highlighted > rect, .node-highlighted > polygon,
        .node-highlighted > circle, .node-highlighted > ellipse {
            stroke: #00d9ff !important;
            stroke-width: 2.5px !important;
        }
        .cluster-dimmed { opacity: 0.2 !important; }
    </style>
</head>
<body>
    <div class="container">
    ...
    cat >> "$output_file" << HTMLEOF
        ...
        <div class="beta-banner">
            ...
            <span class="beta-text">
                ...
                <br>💡 <strong>Click a node</strong> to highlight its connections. Scroll or drag to navigate.
            </span>
        </div>
        ...
        <div class="graph-wrapper">
            <div class="zoom-controls">
                ...
                <button class="zoom-btn" id="btn-zoom-fit" title="Fit to screen" style="font-size:0.75rem;">⤢</button>
            </div>
...
HTMLEOF
    ...
    cat >> "$output_file" << 'HTMLEOF'
        ...
        <div class="legend">
            <div class="legend-item"><span>🔷</span><span>Go Service</span></div>
            <div class="legend-item"><span>🐳</span><span>Docker</span></div>
            <div class="legend-item"><span>🏗️</span><span>Terraform</span></div>
            <div class="legend-item"><span>λ</span><span>Lambda</span></div>
            <div class="legend-item"><span>📦</span><span>Node.js</span></div>
            <div class="legend-item"><span>☁️</span><span>Cloud Provider</span></div>
        </div>
    ...
    <script type="module">
        ...
        import elkLayouts from 'https://cdn.jsdelivr.net/npm/@mermaid-js/layout-elk@0.1.6/dist/mermaid-layout-elk.esm.min.mjs';
        ...
        function setupHighlight(svg) {
            // Build adjacency from edge paths
            // Mermaid adds data-id on .node and edge labels on .edgePath
            const nodes   = svg.querySelectorAll('.node');
            const edges   = svg.querySelectorAll('.edgePath');
            const edgeLbls= svg.querySelectorAll('.edgeLabel');
            const clusters= svg.querySelectorAll('.cluster');

            // Map node id → node element
            const nodeMap = {};
            nodes.forEach(n => {
            ...
            // Build forward/backward adjacency from flowchart edge data
            // Mermaid encodes edge source/target in the path id: "L-source-target-N"
            const adjacency = {}; // id → { out: Set, in: Set }
            const ensureNode = id => {
                if (!adjacency[id]) adjacency[id] = { out: new Set(), in: new Set() };
            };

            edges.forEach(ep => {
                const pathId = ep.id || '';
                // format: L-sourceId-targetId-0
                const m = pathId.match(/^L-(.+?)-(.+?)-\d+$/);
                if (m) {
                    const [, src, tgt] = m;
                    ensureNode(src); ensureNode(tgt);
                    adjacency[src].out.add(tgt);
                    adjacency[tgt].in.add(src);
                }
            });
            ...
            function clearHighlight() {
                ...
                edges.forEach(e => e.classList.remove('edge-dimmed'));
                edgeLbls.forEach(e => e.classList.remove('edge-dimmed'));
                clusters.forEach(c => c.classList.remove('cluster-dimmed'));
                infoPanel.classList.remove('visible');
                activeId = null;
            }

            infoClose.addEventListener('click', clearHighlight);
            ...
            nodes.forEach(node => {
                ...
                node.addEventListener('click', (e) => {
                    e.stopPropagation();
                    const rawId = node.id || '';
                    // Mermaid node id format: "flowchart-realname-N"
                    const idMatch = rawId.match(/flowchart-(.+?)-\d+/) || rawId.match(/(.+)/);
                    const nodeId = idMatch ? idMatch[1] : rawId;

                    if (activeId === nodeId) { clearHighlight(); return; }
                    activeId = nodeId;

                    const adj = adjacency[nodeId] || { out: new Set(), in: new Set() };
                    const related = new Set([nodeId, ...adj.out, ...adj.in]);

                    // Collect which edge paths touch this node
                    const relatedEdges = new Set();
                    edges.forEach(ep => {
                        const m = (ep.id || '').match(/^L-(.+?)-(.+?)-\d+$/);
                        if (m && (m[1] === nodeId || m[2] === nodeId)) {
                            relatedEdges.add(ep.id);
                        }
                    });

                    // Apply dim/highlight
                    nodes.forEach(n => {
                        const rid = (n.id || '').match(/flowchart-(.+?)-\d+/);
                        const nid = rid ? rid[1] : (n.id || '');
                        if (related.has(nid)) {
                            n.classList.remove('node-dimmed');
                            if (nid === nodeId) n.classList.add('node-highlighted');
                            else n.classList.remove('node-highlighted');
                        } else {
                    ...
                    edges.forEach(ep => {
                        if (relatedEdges.has(ep.id)) ep.classList.remove('edge-dimmed');
                        else ep.classList.add('edge-dimmed');
                    });
                    edgeLbls.forEach(el => {
                        // dim all labels not connected — match by position heuristic
                        el.classList.add('edge-dimmed');
                    });

                    // Show info panel
                    const label = node.querySelector('text, .label')?.textContent?.trim() || nodeId;
                    infoTitle.textContent = label;
                    let html = '';
                    if (adj.out.size) {
                        html += `<div class="conn-label">Connects to</div>`;
                        adj.out.forEach(id => { html += `<div class="conn-item">→ ${id}</div>`; });
                    }
                    if (adj.in.size) {
                        html += `<div class="conn-label">Connected from</div>`;
                        adj.in.forEach(id => { html += `<div class="conn-item">← ${id}</div>`; });
                    }
                    if (!adj.out.size && !adj.in.size) {
                        html = '<div style="opacity:0.5">No detected connections</div>';
                    }
